...
if [ -n "${RENDER_EXTERNAL_URL:-}" ]; then
  RENDER_GATEWAY_URL="${RENDER_EXTERNAL_URL%/}/openclaw"
  RENDER_GATEWAY_WS_URL="$(printf '%s' "$RENDER_GATEWAY_URL" | sed 's#^https:#wss:#; s#^http:#ws:#')"
  export NANOCLAW_OPENCLAW_GATEWAY_PUBLIC_WS_URL="$RENDER_GATEWAY_WS_URL"
  export NANOCLAW_OPENCLAW_GATEWAY_PUBLIC_HEALTH_URL="${RENDER_GATEWAY_URL}/health"
fi
