...
| `src/orchestrator/supervisor.rs`, `src/orchestrator/healing.rs`, `src/orchestrator/homeostasis.rs`, `src/orchestrator/sensory.rs` | `assimilate-runtime` | `nanoclaw runtime status|poll|serve` plus future runtime health loop | Keep the harness idea of a supervisor, not the old supervisor type. |
...
2. Runtime supervisor: extend `nanoclaw runtime status|poll|serve` with
   lifecycle inspection, stop/reload semantics, and health-loop reporting.
