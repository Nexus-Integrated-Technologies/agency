...
| `src/orchestrator/supervisor.rs`, `src/orchestrator/healing.rs`, `src/orchestrator/homeostasis.rs`, `src/orchestrator/sensory.rs` | `assimilate-runtime` | `nanoclaw runtime status|inspect|poll|serve` plus future runtime health loop | Keep the harness idea of a supervisor, not the old supervisor type. |
...
1. Evidence and closure: fold FPF evidence, gate, assurance, provenance, and
   ...
   success returns, and scheduled-task completion now requires successful
   validated evidence. Remaining work is failure blockers, explicit
   verification commands, and non-local issue/writeback closure gating.
