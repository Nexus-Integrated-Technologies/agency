...
| `src/orchestrator/supervisor.rs`, `src/orchestrator/healing.rs`, `src/orchestrator/homeostasis.rs`, `src/orchestrator/sensory.rs` | `assimilate-runtime` | unified `nanoclaw runtime` command and runtime health loop | Keep the harness idea of a supervisor, not the old supervisor type. |
...
| `src/models/*`, `src/services/{speaker,listener}.rs`, `docker/Dockerfile.speaker` | `park-reference` | future media/audio lane if required | Model/audio services are not part of the NanoClaw control-plane collapse. |
...
| inactive `src/bin/*` and `tests/*` | `park-reference` | `graveyard/agency-harness/` | Preserved as source material but removed from active Cargo discovery paths. |
...
2. Runtime supervisor: distill old supervisor/healing/homeostasis ideas into a
   `nanoclaw runtime` command that owns local, Slack, webhook, and OpenClaw
   loops.
