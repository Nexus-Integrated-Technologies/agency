#!/usr/bin/env python3
"""Run a paid Azure AI Foundry MaaS smoke through NanoClaw's worker contract.
...
def build_request() -> dict:
    ...
    session_id = "foundry-maas-paid-smoke-" + uuid.uuid4().hex[:8]
    root = os.path.join(tempfile.gettempdir(), session_id)
    message = os.environ.get("NANOCLAW_FOUNDRY_MAAS_SMOKE_PROMPT", "").strip()
    if not message:
        message = "Output token foundry-maas-paid-smoke-ok only."

    return {
        "invocation_id": "exec-" + session_id,
        "requested_at": now,
        "group": {
            "jid": "paperclip:smoke:foundry-maas",
            "name": "Foundry MaaS Paid Smoke",
            "folder": "paperclip_agent_foundry_maas_paid_smoke",
            "trigger": "@nanoclaw",
            "added_at": now,
        ...
        "messages": [
            {
                "id": "msg-" + session_id,
                "chat_jid": "paperclip:smoke:foundry-maas",
                "sender": "paperclip",
        ...
        "session": {
            "id": session_id,
            "group_folder": "paperclip_agent_foundry_maas_paid_smoke",
            "workspace_root": root,
...
def main() -> int:
    ...
    timeout = int(os.environ.get("NANOCLAW_FOUNDRY_MAAS_SMOKE_TIMEOUT_SECONDS", "180"))
    proc = subprocess.run(
