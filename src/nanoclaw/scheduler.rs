...
mod tests {
    ...
    use super::{build_scheduled_task, compute_next_run, TaskScheduleInput};
    ...
    #[test]
    fn cron_task_computes_next_occurrence() {
        let now = Utc.with_ymd_and_hms(2026, 4, 5, 12, 0, 0).unwrap();
        let task = build_scheduled_task(
            TaskScheduleInput {
                group_folder: "main".to_string(),
        ...
        assert_eq!(task.next_run.as_deref(), Some("2026-04-05T13:00:00+00:00"));
    }
}
